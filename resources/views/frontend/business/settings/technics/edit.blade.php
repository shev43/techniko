    ...
    <main class="main page">
        ...
        <div class="container">
            ...
            <section class="seller_cabinet-settings-technics-create">
                <h2 class="title text-center">Редагувати техніку</h2>

                <form id="businessProductForm" class="form-prevent"
                      action="{{ route('business::settings.technics.update', ['lang'=>app()->getLocale(), 'technic_id'=>$technic->id]) }}"
                        ...
                        <div class="col-12 col-md-4">
                            <div class="form-group">
                                ...
                                <input class="form-control basicAutoComplete @error('name') is-invalid @enderror"
                                       name="name" type="text" autocomplete="off" value="{{ old('name', $technic->name) }}">
                            </div>
                        </div>
                        <div class="col-12 col-md-4">
                            <div class="form-group">
                                ...
                                <select class="selectpicker @error('machine') is-invalid @enderror" name="machine"
                                    ...
                                    @foreach($machineCategories as $category)
                                        <option
                                            value="{{ $category->id }}" {{ old('machine', $technic->machine_id) === $category->id ? 'selected' : '' }}>{{ $category->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-12 col-md-4">
                            <div class="form-group">
                                ...
                                <select class="selectpicker @error('type_of_delivery') is-invalid @enderror" name="type_of_delivery"
                                    ...
                                    <option value="self" {{ $technic->type_of_delivery == 'self' ? 'selected' : '' }}>Самовивіз</option>
                                    <option value="business" {{ $technic->type_of_delivery == 'business' ? 'selected' : '' }}>Доставка за адресою</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <select class="selectpicker @error('related_categories') is-invalid @enderror" name="related_categories[]"
                                    ...
                                    <option value="{{ $machine->id }}" @if(in_array($machine->id, explode(',', $technic->related_categories))) selected @endif >{{ $machine->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="d-flex align-items-center">
                                    <input class="form-control text-center @error('price') is-invalid @enderror"
                                           ...
                                           step="1"
                                           style="width: calc(100% - 100px);margin-right: 20px;" value="{{ old('price', $technic->price) }}">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="selectpicker  @error('is_driver') is-invalid @enderror" name="is_driver"
                                        ...
                                        <option value="1" @if($technic->is_driver == 1) selected @endif>Так</option>
                                        <option value="0" @if($technic->is_driver == 0) selected @endif >Ні</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <div @error('description') style="border: 1px solid #FC610A;" @enderror>
                                    <textarea class="form-control editor" id="business-description"
                                              name="description">{{ old('description', $technic->description) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <input type="text"
                                       ...
                                       placeholder="Вкажіть повну адресу підприємства" value="{{ old('address', $technic->address) }}">
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                        <div class="col-12">
                            @include('frontend._modules.map-form', ['objects'=>$technic, 'target'=>'form#businessProductForm input[name=address]'])
                        </div>
                    ...
                    <div class="row js-product-images-block">
                        ...
                        @foreach($technic->photo as $photo)
                            <div class="col-md-4 js-product-media-item">
                                <div class="technic-image position-relative">
                                    <img src="{{ asset('/storage/technics/'.$photo->photo) }}" alt="">
                                    <input id="media-file" type="hidden" name="media[][src]" value="{{ $photo->photo }}" >
